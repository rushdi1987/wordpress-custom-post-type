...
class CPT {
    ...
    function __construct( $post_type_names, $options = array() ) {
        ...
        // Register the post type custom metabox.
        $this->add_action( 'add_meta_boxes', array( &$this,'add_tm_custom_post_metaboxes' ) );
        $this->add_action('save_post', array( &$this,'tm_save_metadata' ), 1, 2);
...
>>TM metaboxes 
*/
function add_tm_custom_post_metaboxes( $metabox_name ) {
    $metabox_name = $this->metabox_name ;
   	add_meta_box('tm_meatabox_input', $metabox_name, 'tm_meatabox_input', 'member', 'side', 'default');



    // The Custom Post Metabox

    function tm_meatabox_input() {
        global $post;
        
        // Noncename needed to verify where the data originated
        echo '<input type="hidden" name="metadata_noncename" id="metadata_noncename" value="' . 
        wp_create_nonce( plugin_basename(__FILE__) ) . '" />';
        
        // Get the metaValue data if its already been entered
        $metavalue = get_post_meta($post->ID, '_metadata', true);
        
        // Echo out the field
        echo '<input type="text" name="_metadata" value="' . $metavalue  . '" class="widefat" />';

    }
}
// Save the Metabox Data
function tm_save_metadata($post_id, $post) {
    
    // verify this came from the our screen and with proper authorization,
    // because save_post can be triggered at other times
    if ( !wp_verify_nonce( $_POST['metadata_noncename'], plugin_basename(__FILE__) )) {
    return $post->ID;
    }
    ...
    $metadata['_metadata'] = $_POST['_metadata'];
    
    // Add values of $metadata as custom fields
    
    foreach ($metadata as $key => $value) { // Cycle through the $metadata array!
